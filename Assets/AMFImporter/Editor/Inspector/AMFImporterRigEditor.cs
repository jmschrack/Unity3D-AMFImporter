...
using UnityEditor;
    ...
    internal class AMFImporterRigEditor : BaseAssetImporterTabUI
    {
        const float kDeleteWidth = 17;

        AMFImporter singleImporter { get { return targets[0] as AMFImporter; } }

        public int m_SelectedClipIndex = -1;

        Avatar m_Avatar;
        SerializedProperty m_eulerOffset;
        SerializedProperty m_rootOffset;
        ...
        static class Styles
        {
            public static GUIContent AnimationType = new GUIContent("Animation Type", "The type of animation to support / import.");
        ...
        public AMFImporterRigEditor(ScriptedImporterEditor panelContainer)
            : base(panelContainer)
        {}

        internal override void OnEnable()
        {
            m_eulerOffset=serializedObject.FindProperty("rigEulerRoot");
            m_rootOffset=serializedObject.FindProperty("rigOffset");
            
            m_AnimationType = serializedObject.FindProperty("rigType");
            ...
            */
            // Animation
            m_CopyAvatar = serializedObject.FindProperty("copyAvatar");
           ...
           /*  if (m_AnimationType.intValue == (int)ModelImporterAnimationType.Human)
            {
                ...
                if (go != null)
                {
                    m_IsBiped = false;//AvatarBipedMapper.IsBiped(go.transform, m_BipedMappingReport);
                }
            } */
        }
        ...
        void LegacyGUI()
        {
            m_LegacyGenerateAnimations.intValue=EditorGUILayout.Popup(Styles.AnimLabel,m_LegacyGenerateAnimations.intValue, Styles.AnimationsOpt);
                ...
                if (!m_CopyAvatar.boolValue)
                {
                   /*  // Do not allow multi edit of root node if all rigs doesn't match
                    ...
                    if (EditorGUI.EndChangeCheck())
                    {
                        ...
                        {
                            //m_RootMotionBoneName.stringValue = FileUtil.GetLastPathNameComponent(m_RootMotionBoneList[rootIndex].text);
                        }
                        else
                        {
...
With this option, this model will not create any avatar but only import animations.", EditorStyles.helpBox);
            ...
            EditorGUI.BeginChangeCheck();
            EditorGUILayout.PropertyField(m_AvatarSource);//TODO "source"
 ...
 */
            ...
            // Animation type
            EditorGUI.BeginChangeCheck();
            m_AnimationType.intValue=EditorGUILayout.Popup(Styles.AnimationType,m_AnimationType.intValue, Styles.AnimationTypeOpt);
            m_rootOffset.vector3Value=EditorGUILayout.Vector3Field("Root Offset",m_rootOffset.vector3Value);
            m_eulerOffset.vector3Value=EditorGUILayout.Vector3Field("Euler Root Offset",m_eulerOffset.vector3Value);
            if (EditorGUI.EndChangeCheck())
            {
                 m_AvatarSource.objectReferenceValue = null;
            ...
            if (!m_AnimationType.hasMultipleDifferentValues)
            {
                ...
                if (m_AnimationType.enumValueIndex == (int)AMFImporter.RigType.Humanoid)
                    HumanoidGUI();
                else if (m_AnimationType.enumValueIndex == (int)AMFImporter.RigType.Generic)
                    GenericGUI();
                /* else if (animationType == ModelImporterAnimationType.Legacy)
                    LegacyGUI(); */
            }else{
                EditorGUILayout.LabelField("Somehow multi object?");
            }
...
/* 
            ...
            if (canOptimizeGameObjects)
            {
                ...
                if (m_OptimizeGameObjects.boolValue &&
                    serializedObject.targetObjects.Count() == 1) // SerializedProperty can't handle multiple string arrays properly.
                {
