...
{
    public class AvatarBipedMapper
    {
        ...
        public static Dictionary<int, Transform> MapBones(Transform root, List<string> report=null)
        {
            Dictionary<int, Transform> ret = new Dictionary<int, Transform>();

            Transform[] humanToTransform = new Transform[HumanTrait.BoneCount];

            if (MapBipedBones(root, ref humanToTransform, report))
            {
                ...
                Debug.Log("MapBipedBones returned false");
            }
            Debug.Log("Human to transform:"+humanToTransform.Length);
            // Move upper chest to chest if no chest was found
            if (!ret.ContainsKey((int)HumanBodyBones.Chest) && ret.ContainsKey((int)HumanBodyBones.UpperChest))
            {
